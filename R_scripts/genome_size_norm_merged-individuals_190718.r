...
kraken2_otu <- read.delim(file="/home/edvo1850/DENTAL_CALC/DENTAL_CALCULUS_JENA_GORILLA_181009/P7_kraken2_merged-indiviuals_190717/kraken2_otu_table_merged_individuals_1908718.txt", sep="\t", skip=1)
...
write.csv(kraken2.nodata, "tax_reports_merged_individuals/190718_no_data.csv", quote = FALSE, row.names = FALSE)
kraken2.nodata <- read.delim("tax_reports_merged_individuals/tax_report_prok.txt")
...
sum(is.na(kraken2.nodata$Average_Genome_Size)) #596
...
length(kraken2.nodata3$Taxon_ID) #308

write.csv(kraken2.nodata3$Taxon_ID, "tax_reports_merged_individuals/190718_no_data_viruses.csv", quote = FALSE, row.names = FALSE)
kraken2.nodata3 <- read.delim("tax_reports_merged_individuals/taxreports_no_data_viruses.txt")
...
length(kraken2.nodata3$taxid) #308
...
length(unique(kraken2.NA$Taxon_ID)) #308 taxa for which I could not obtain average genome size.
...
length(unique(kraken2.NA.2$Taxon_ID)) #246
...
write.csv(kraken2.nodata4, "tax_reports_merged_individuals/190718_no_data_rest.csv", quote = FALSE, row.names = FALSE)
kraken2.nodata4 <- read.delim("tax_reports_merged_individuals/tax_report_rest.txt")
...
length(kraken2.nodata4$taxid) #246
...
sum(is.na(kraken2.nodata4$Average_Genome_Size)) #215
...
length(kraken2.nodata5$taxid) #215
...
write.csv(kraken2.nodata6$Taxon_ID, "tax_reports_merged_individuals/190718_no_data_viruses_rest.csv", quote = FALSE, row.names = FALSE)
kraken2.nodata6 <- read.delim("tax_reports_merged_individuals/tax_report_virus_rest.txt")
...
kraken2.NA <- kraken2_otu[which(is.na(kraken2_otu$Average_Genome_Size)),]
length(unique(kraken2.NA$Taxon_ID)) #142
kraken2.NA.taxa <- as.data.frame(unique(kraken2.NA$Taxon_ID))
write.csv(kraken2.NA.taxa, "Rout_merged_individuals/Taxa_without_average_genome_size_190718.csv")


kraken2_otu$Ratio <- kraken2_otu$Raw_Abundance / kraken2_otu$Average_Genome_Size
sum(is.na(kraken2_otu$Average_Genome_Size)) #2982
...
write.table(mean.av.genome.size, "Rout_merged_individuals/average_genome_sizes.txt")
...
write.csv(kraken2_otu_out, "Rout_merged_individuals/Rout_post_genome_normalisation_merged_individuals_190718.csv", quote = FALSE, row.names = TRUE)
